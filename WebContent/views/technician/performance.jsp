...
<div id="wrapper">
   ...
   <table class="table">
     <thead>
      ...
      <tbody>
      <c:forEach items="${perform}" var = "perform" >
      <tr>
          <td> ${fn:substring(perform.day,0,4)}年${fn:substring(perform.day,4,6)}月${fn:substring(perform.day,6,8)}日 </td>
          <td>${perform.spaAmount}</td>
          <td>${perform.massAmount}</td>
          <td>${perform.cupAmount}</td>
          <td>${perform.salesVolume}</td>
      </tr>
      </c:forEach> 
      </tbody>
    </table>
